  ...
  onLoad: function (options) {
    ...
    wx.getStorage({
      ...
      success: function(res) {
        if (res.data >= time)
           {
          if (app.globalData.openid =="olrgP0RWK3g5tv39iFf_eAw4YP-E")
          {
            wx.showModal({
              title: '投票状态',
              content: '投票已结束，是否公布',
    ...
    var that = this;
    wx.request({
      url: 'http://www.ownersbuild.com:3000/vote/detail', //仅为示例，并非真实的接口地址
      data: {
        voteTitle: voteTitle,
        voteDate: voteDate
      },
    ...
    wx.request({
      url: 'http://www.ownersbuild.com:3000/people', //仅为示例，并非真实的接口地址
      data: {},
      method: 'POST',
      header: {
        'content-type': 'application/json'
    ...
    wx.getStorage({
      ...
      success: function(res) {
        if (that.data.voteTitle==res.data){
          ...
          wx.request({
            url: 'http://www.ownersbuild.com:3000/vote/change',
            data: {
